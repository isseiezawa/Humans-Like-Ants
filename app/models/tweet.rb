...
class Tweet < ApplicationRecord
  ...
  def tweet_to_hash
    ...
    image.attached? ? as_json(methods: [:image_url], only: [:post], include: { user: { methods: [:avatar_url], only: [:name] } }) : as_json(only: [:post], include: { user: { methods: [:avatar_url], only: [:name] } })
  end
